...
def find_parent(parent, x):
  if parent[x] != x:
    parent[x] = find_parent(parent, parent[x]) # 재귀호출로 부모 노드를 거슬러 가는 과정에서 부모 테이블을 루트 테이블로 갱신하고 루트를 리턴함.
  return parent[x]

def union_parent(parent, a, b): # 집합에 포함시키는 union 함수(= 최종적으로 그래프에서 disjoint set을 만드는 함수. 그래프에서 부모 테이블을 고려하면 disjoint set임).
...
for i in range(e): # union 연산(= 간선 정보 = 서로소 집합 정보) 입력대로 수행
...
for i in range(e):
  ...
  if find_parent(parent, a) == find_parent(parent, b): # 입력받은 간선 (a, b)에서 각각의 원소의 루트 노드가 같다면, 사이클이 존재
    ...
    union_parent(parent, a, b) # ?? 걍 다 추가해놓고 하면 되는거 아닌가.. 사이클을 제외한 서로소 집합을 구성하기 위함?
